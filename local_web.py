...
def main():
    ...
    if data is None:
        ...
        for row in data:
            message = "I found a session called '%s' by %s at %s" % (row[1], row[6], row[9])
...
def ask_if_speaker(speaker_name):
    ...
    if data is None:
        ...
        for row in data:
            message = "Welcome to Conference Info: I found a session called '%s' by %s at %s" % (row[1], row[6], row[9])
